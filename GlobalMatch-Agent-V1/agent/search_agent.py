...
# 爬虫模块映射
SCRAPER_MAP = {
    "importyeti": "scrapers.importyeti_scraper",
    "kompass": "scrapers.kompass_scraper",
    "alibaba_rfq": "scrapers.alibaba_rfq_scraper",
    "google": "scrapers.google_scraper",
    "europages": "scrapers.europages_scraper",
    "yellowpages": "scrapers.yellowpages_scraper",
    "tradeindia": "scrapers.tradeindia_scraper",
    "ec21": "scrapers.ec21_scraper",
    "linkedin": "scrapers.linkedin_scraper",
    "piers": "scrapers.piers_scraper",
    "thomasnet": "scrapers.thomasnet_scraper",
    "hoovers": "scrapers.hoovers_scraper",
    "wlw": "scrapers.wlw_scraper",
    "kellysearch": "scrapers.kellysearch_scraper",
    "zawya": "scrapers.zawya_scraper",
    "tradearabia": "scrapers.tradearabia_scraper",
    "exportersindia": "scrapers.exportersindia_scraper",
    "tradekey": "scrapers.tradekey_scraper",
    "tradewheel": "scrapers.tradewheel_scraper",
    "exporthub": "scrapers.exporthub_scraper",
}
...
class SearchAgent:
    ...
    async def search(self, search_params: dict, target_count: int = 100) -> list[dict]:
        ...
        async def run_scraper(source_name: str) -> list[dict]:
            ...
            async with source_semaphore:
                ...
                return await self._run_scraper_isolated(
                    source_name, keywords, target_countries, buyer_types
                )
        ...
        if len(valid_buyers) < target_count and not self._is_timeout():
            ...
            expanded = await self._expand_search(
                keywords, valid_buyers, target_count - len(valid_buyers),
                target_countries, buyer_types
            )
            if expanded:
    ...
    async def _run_scraper_isolated(self, source_name: str, keywords: list[str],
        ...
        """
        try:
            module_path = SCRAPER_MAP.get(source_name)
            if not module_path:
                return []

            module = importlib.import_module(module_path)
            scraper_class = getattr(module, "scrape", None)
            if not scraper_class:
                return []

            # 每个爬虫限制50条结果
            results = await asyncio.wait_for(
                scraper_class(keywords, countries, buyer_types),
                timeout=60.0  # 单个爬虫最长60秒
            )
            ...
            logger.debug("爬虫模块未找到", source=source_name)
            return []
        except Exception as e:
            logger.error("爬虫异常，跳过", source=source_name, error=str(e))
            return []

    async def _expand_search(self, keywords: list[str], existing: list[dict],
                              need: int, countries: list[str],
